...
class IMG_Stegno:
    ...
    #Geeta- main frame or start page
    def main(self, root):
        root.title('ImageSteganography by Geeta')
        root.geometry('500x600')
        root.resizable(width =False, height=False)
        root.config(bg = '#F9F4D9')
        frame = Frame(root)
        frame.grid()
        
        title = Label(frame,text='Geeta Image Steganography')
        title.config(font=('Times new roman',25, 'bold'))
        title.grid(pady=10)
        title.config(bg = '#F9F4D9')
        title.grid(row=1)

        encode = Button(frame,text="Encode",command= lambda :self.encode_frame1(frame), padx=14,bg = '#F9F4D9' )
        encode.config(font=('Helvetica',14), bg='#F3E7A7')
        encode.grid(row=2)
        decode = Button(frame, text="Decode",command=lambda :self.decode_frame1(frame), padx=14,bg = '#F9F4D9')
        decode.config(font=('Helvetica',14), bg='#F3E7A7')
        decode.grid(pady = 12)
        decode.grid(row=3)
    ...
    #Geeta- back function to loop back to main screen
    def back(self,frame):
        frame.destroy()
        self.main(root)

    
    #Geeta- frame for encode page
    def encode_frame1(self,F):
        F.destroy()
        F2 = Frame(root)
        label1= Label(F2,text='Select the Image in which \nyou want to hide text :')
        label1.config(font=('Times new roman',25, 'bold'),bg = '#F9F4D9')
        label1.grid()

        button_bws = Button(F2,text='Select',command=lambda : self.encode_frame2(F2))
        button_bws.config(font=('Helvetica',18), bg='#F3E7A7')
        button_bws.grid()
        button_back = Button(F2, text='Cancel', command=lambda : IMG_Stegno.back(self,F2))
        button_back.config(font=('Helvetica',18),bg='#F3E7A7')
        button_back.grid(pady=15)
        button_back.grid()
        F2.grid()

    #Geeta- frame for decode page
    ...
    #Geeta- function to encode image 
    def encode_frame2(self,e_F2):
        ...
        if not myfile:
            ...
            encode_button.config(font=('Helvetica',14), bg='#F3E7A7')
            ...
            button_back.config(font=('Helvetica',14), bg='#F3E7A7')
            button_back.grid(pady=15)
            encode_button.grid()
            e_pg.grid(row=1)
            e_F2.destroy()
    ...
    #Geeta- function to decode image 
    ...
    #Geeta- function to generate data
    ...
    #Geeta- function to modify the pixels of image
    ...
    #Geeta- function to enter the data pixels in image
    ...
    #Geeta- function to enter hidden text
